...
public class ClipImageView extends ImageView implements View.OnTouchListener, ViewTreeObserver.OnGlobalLayoutListener{
    ...
    public Bitmap clipForRct(float aspectRatio ){
        Bitmap bitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap);
        draw(canvas);
        return Bitmap.createBitmap(bitmap, (getWidth() - borderlength) / 2, (int) ((getHeight()- borderlength/aspectRatio) / 2), borderlength, (int) (borderlength*0.525));
    }
